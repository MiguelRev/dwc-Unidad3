...
const mes:number =Number( prompt("Introduce el número de mes de tu cumpleaños"));
const dia:number = Number(prompt("Introduce el número del día de tu cumpleaños"));
...
if (isNaN(mes) || isNaN(mes) || mes > 12 || mes < 0 || dia > 31 || dia < 1) {
    ...
    let anho:number = fecha.getFullYear();
    while (anho < 2100) {
